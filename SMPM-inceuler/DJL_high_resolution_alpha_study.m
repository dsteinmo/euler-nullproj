...
% Cornell University.
   ...
   for ii = 1:length(alpha1)
         ...
         % Loop over the different values of tau we want to use.
         for jj = 1:length(alpha2)
            ...
            err_rel_u(ii,jj)   = norm( diff_ux ) / norm( ux0(:, 1:end - ndx ) );
            err_rel_w(ii,jj)   = norm( diff_uz ) / norm( uz0(:, 1:end - ndx ) );
            err_rel_rho(ii,jj) = norm ( diff_rho ) / norm( rhoi(:, 1:end - ndx ) );;
            err_u(ii,jj)   = norm( diff_ux );
            err_w(ii,jj)   = norm( diff_uz );
            err_rho(ii,jj) = norm ( diff_rho );
keyboard;
