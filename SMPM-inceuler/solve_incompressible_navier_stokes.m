function [ux uz rho t] = solve_incompressible_navier_stokes( n, mx, mz, x, z, ux0, uz0, rhoi, rhob, max_dt, t_final, ptype, tau, nu, bc_viscous, bc_diffusion, ux_b, uz_b )
...
%     nu             - the viscosity and the diffusivity (assumed to be the same).
%     bc_viscous     - 'free' or 'no' for free-slip and no-slip.
%     bc_diffusion   - 'd' or 'n' for dirichlet and neumann.
%     ux_b, uz_b     - boundary condition values for non-homogenous Dirichlet boundaries (grid size/shape arrays).
%
%  Returns 3 arguments:
%
...
%  Cornell University
   ...
   % Build homogenous boundary condition values if none were given.
   if nargin < 15
      ux_b = 0 * ux0;
      uz_b = 0 * uz0;
   end
...
%            fprintf([ '   GMRES converged in ', num2str( m ) ' iterations.\n'] );
      ...
      % Set non-homogenous boundary conditions for Dirichlet boundaries.
      if strcmp( bc_viscous, 'no' )
         [iiux iiuz] = apply_smpm_boundary_conditions( iiux, iiuz, ux_b, uz_b, n, mx, mz, Lx, Lz, nu )
      end

      % Solve the viscous and diffusive equations.
      iiux  = ( speye( r, r ) / dt / nu - Lux )  \ ( iiux  / ( dt * nu ) );
      iiuz  = ( speye( r, r ) / dt / nu - Luz )  \ ( iiuz  / ( dt * nu ) );
      iirho = ( speye( r, r ) / dt / nu - Lrho ) \ ( iirho / ( dt * nu ) );
