function [curvedElements,blendfact_x,blendfact_y] = MakeCurvedEdges_derek(BCFlag,xs,ys,t)
...
blendfact_x = zeros(Np,K);
blendfact_y = zeros(Np,K);
...
for n= curvedFaces
  ...
  blendfact_x(ids,k) = blend.*vdx(ids);
  blendfact_y(ids,k) = blend.*vdy(ids);
  
  x(:,k) = x(:,k) + blendfact_x(:,k);
  y(:,k) = y(:,k) + blendfact_y(:,k);
