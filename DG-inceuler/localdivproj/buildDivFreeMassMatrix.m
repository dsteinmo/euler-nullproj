function [mat,upsi,vpsi] = buildDivFreeMassMatrix(N,r,s,V,jac,rx,ry,sx,sy)
    %[psir,psis] = GradVandermonde2D(N-2,r,s);
    [psir,psis] = GradVandermonde2D(N,r,s); %this seems like best bet.
    ...
    for ii=1:Npdiv
        for jj=1:Npdiv
            ...
            mat(ii,jj) = coefs(1)*(2/sqrt(2)); %integrate
        end
    end
end