...
function result = pressproj_hnd_uvonly_visc(RHS,dt,ll,uu,pp,qq,llv,uuv,ppv,qqv)
    %Globals2D;
...
%   
  ...
  RHSu = MassMatrix*(J.*u); %forgot about mass matrix shit.
  RHSv = MassMatrix*(J.*v); %forgot about mass matrix shit.
  
  u = qqv*(uuv\(llv\(ppv*RHSu(:))));
  v = qqv*(uuv\(llv\(ppv*RHSv(:))));
%   u=reshape(u,Np,K);
%   v=reshape(v,Np,K);
%   Qnp1(:,:,2)=u;
%   Qnp1(:,:,3)=v;
