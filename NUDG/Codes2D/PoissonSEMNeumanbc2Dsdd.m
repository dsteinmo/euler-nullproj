function [bc] = PoissonIPDGbc2Dsdd(ubc,qbc,a)
...
for k1=1:K 
  ...
  for f1=1:Nfaces
    ...
    if(BCType(k1,f1))
      ...
      %still need to check the following is right with an inhomogenous test
      %of PoissonDriver2D.m with inhomogenous BC's and space-dep.
      %diffusivity
      
      switch(BCType(k1,f1))
		  case {Neuman}
	      bc(:,k1) = bc(:,k1) + mmE(:,Fm1)*rtak1mat(Fm1,Fm1)*qbc(fidM);
          otherwise
	  end 
    end   
  end  
