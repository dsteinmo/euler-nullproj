function [OP,MM] = PoissonIPDG2Dquad()
...
V1D = Vandermonde1D(N, faceS);  massEdge(Fm,Fm,2) = inv(V1D*V1D');
Fm = Fmask(:,3); faceR = r(Fm); 
V1D = Vandermonde1D(N, faceR);  massEdge(Fm,Fm,3) = inv(V1D*V1D');
Fm = Fmask(:,4); faceS = s(Fm);
V1D = Vandermonde1D(N, faceS);  massEdge(Fm,Fm,4) = inv(V1D*V1D');
...
for k1=1:K 
  ...
  for f1=1:Nfaces
    ...
    %gtau = 1e8;
    %gtau = -2e9*(N+1)*(N+1)*hinv;
    gtau = 100*2*(N+1)*(N+1)*hinv; % set penalty scaling
    %gtau = gtau*1e6; %seems to give better agreement
